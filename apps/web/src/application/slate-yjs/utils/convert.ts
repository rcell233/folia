...
import { BlockData, BlockType, YDoc, YjsEditorKey, YSharedRoot } from '@/application/types';
...
export function traverseBlock(id: string, sharedRoot: YSharedRoot): Element | undefined {
  ...
  const children = (childrenMap.get(childrenId)?.toJSON() ?? [])
    .map((childId: string) => {
...
export function slateContentInsertToYData(
  ...
  function processNode(node: Element, parentId: string, index: number) {
    ...
    const [textNode, ...children] =
      (node.children[0] as Element).type === 'text'
        ? [node.children[0] as Element, ...node.children.slice(1)]
        : [null, ...node.children];
